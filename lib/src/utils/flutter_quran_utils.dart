...
import '/src/models/ayah.dart';
import '/src/models/bookmark.dart';
import '/src/models/surah_info_style.dart';
...
class FlutterQuran {
  ...
  Future<void> init(
    ...
    QuranCtrl.instance.state.isDownloadedV2Fonts.value =
        GetStorage().read(Preferences().isDownloadedCodeV2Fonts) ?? false;
    GetStorage().read(Preferences().isDownloadedCodeV2Fonts)
    ...
    QuranCtrl.instance.state.isBold.value =
        GetStorage().read(Preferences().isBold) ?? 0;
  ...
  /// [jumpToJozz] let's you navigate to any quran jozz with jozz number
  /// Note it receives jozz number not jozz index
  void jumpToJozz(int jozz) =>
  ...
  /// [allJozz] returns list of all Quran jozz' names
  List<String> get allJozz => QuranConstants.quranHizbs
      .sublist(0, 30)
      .map((jozz) => "الجزء $jozz")
      .toList();
  ...
  void getSurahInfoDialog(SurahInfoStyle surahInfoStyle,
  ...
  /// لمعرفة ما إذا كانت الاخطوط محملة او لا، ما عليك سوى إستدعاء [isFontsDownloaded]
  ///
